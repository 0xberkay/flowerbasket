...
// ChangePass changes password of seller and updates it in db with code = null
func ChangePass(c *fiber.Ctx) error {
	...
	_, err = database.DB.Exec("UPDATE sellers SET code = NULL ,password = ? WHERE sellerId = ?", user.Password, uint(claims["sub"].(float64)))
	if err != nil {
		return errorHandler.InternalServerError(c)
	}

	go database.Lof.Println("Seller | Password changed successfully by", uint(claims["sub"].(float64)))
